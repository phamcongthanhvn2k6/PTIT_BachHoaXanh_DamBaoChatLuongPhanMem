...
export const list = async (req, res) => {
  try {
    ...
    const mappedData = await Promise.all(data.map(async (item) => {
      ...
      const resolvedPricing = await resolveProductPricing(item, item.branch_info || null, branchId, {
        now: minDate,
...
const processProductData = (data) => {
  ...
  if (Array.isArray(data.related_product_ids)) {
    data.related_product_ids = data.related_product_ids.map(normalizeId).filter(Boolean);
  }
  if (Array.isArray(data.frequently_bought_together)) {
    data.frequently_bought_together = data.frequently_bought_together.map(normalizeId).filter(Boolean);
  }
...
export const create = async (req, res) => {
  try {
    ...
    // Validate uniqueness of SKU if provided
    if (body.sku) {
      const existingSku = await Product.findOne({ sku: body.sku, is_deleted: { $ne: true } });
      if (existingSku) {
        return res.status(400).json({ success: false, message: `Mã SKU '${body.sku}' đã tồn tại trên hệ thống.` });
      }
    }

    // Validate uniqueness of Barcode if provided
    if (body.barcode) {
      const existingBarcode = await Product.findOne({ barcode: body.barcode, is_deleted: { $ne: true } });
      if (existingBarcode) {
        return res.status(400).json({ success: false, message: `Mã Barcode '${body.barcode}' đã tồn tại trên hệ thống.` });
      }
    }

    // Validate name fuzzy duplication to prevent "Coca Cola 1.5L" vs "Coca-Cola 1.5 L"
    if (body.name) {
      const normalizeFuzzy = (val) => String(val || '').toLowerCase().replace(/[^a-z0-9]/g, '').trim();
      const newNormalizedName = normalizeFuzzy(body.name);
      
      const allProducts = await Product.find({ is_deleted: { $ne: true } }).select('name').lean();
      const duplicate = allProducts.find(p => normalizeFuzzy(p.name) === newNormalizedName);
      
      if (duplicate) {
        return res.status(400).json({ 
          success: false, 
          message: `Tên sản phẩm trùng hoặc gần trùng với sản phẩm đã có: '${duplicate.name}'. Vui lòng kiểm tra lại.` 
        });
      }
    }
