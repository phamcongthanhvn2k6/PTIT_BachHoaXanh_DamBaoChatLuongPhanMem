...
// The fuzzy matcher replaced some code, let me first see what is there.
const startIndex = content.indexOf('  const validateFullPromotionForm = (): boolean => {');

// wait, the fuzzy matcher created a malformed function.
// Let's find the first `  const validateFullPromotionForm = (): boolean => {`
// And let's find `  const uploadImageIfNeeded = async (): Promise<string> => {`
const endIndex = content.indexOf('  const uploadImageIfNeeded = async (): Promise<string> => {');
